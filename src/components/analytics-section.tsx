      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { title: "Total Savings", value: "$12,580", change: "+8.2%", icon: <TrendingUp />, color: colors.indigo },
              { title: "Monthly Spend", value: "$2,420", change: "-4.3%", icon: <BarChart2 />, color: colors.sky },
              { title: "Budget Status", value: "On Track", change: "92%", icon: <PieChart />, color: colors.rose },
              { title: "Active Goals", value: "4/5", change: "+1", icon: <LineChart />, color: colors.amber }
            ].map((stat, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
                ...
                viewport={{ once: true }}
                className="group relative p-4 rounded-xl bg-card/50 backdrop-blur-sm border border-border/50 hover:border-primary/20 transition-all duration-300"
              >
